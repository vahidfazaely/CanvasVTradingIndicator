...
- **Signal Diagnostic Mode (v3.2.0, default off):** a **data-collection** layer for objectively answering *why* a candidate was accepted or rejected. Every confirmed candidate on a supported timeframe is logged with the exact engine values (4H regime/slope/separation, 1H structure/momentum, entry structure/gap, candle, ADX/ATR%/chasing, full score breakdown, Entry/SL/TP/Risk/R:R) and the **first failed gate** with its actual value (e.g. `ADX 14.2 ✗ (min 99)`). Accepted signals are then tracked across the next `N` confirmed bars: **MFE/MAE in R**, TP1/TP2/SL hits, and outcome classification (`TP1 FIRST` / `TP2 FIRST` / `SL FIRST` / `AMBIGUOUS` / `EXPIRED`). Output is a compact history table + latest-event detail + optional `CVLOG` alerts (own default-off input, separate from trading alerts). Bounded arrays (25/50/100 events) — no files, no unbounded state, **not a backtest** (no `strategy()`, no orders, no P&L). When off, the script is byte-identical to v3.1.0.
...
- **TradingView:** `v3.2.0` — three-layer TF architecture with a **15m / 1H / 4H signal-engine policy**, **Phase 1 quality gates**, the **Phase 2 structural risk engine** (structural SL, risk validation, R-based TP1/TP2), and the **v3.2.0 Signal Diagnostic Mode** (event logger + MFE/MAE outcome tracker) — working and compiling in the Pine Editor.
