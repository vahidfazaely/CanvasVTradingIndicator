...
try {
  ...
  // Approach: Write to clipboard via CDP, then paste
  // Step 1: Write to clipboard using CDP's DOM.setFileInputFiles or Emulation
  // Actually, use the Clipboard.writeText CDP method (Chrome 96+)
  ...
  const t0 = Date.now();
  try {
    await client.send("Clipboard.writeText", { text: pineCode });
    log(`clipboard.writeText completed in ${Date.now() - t0}ms`);
  } catch (e) {
    // Fallback: use navigator.clipboard.writeText
    log(`Clipboard.writeText failed: ${e.message.split("\n")[0]}, trying navigator.clipboard`);
    await page.evaluate(async (text) => {
      await navigator.clipboard.writeText(text);
    }, pineCode);
    log(`navigator.clipboard.writeText completed in ${Date.now() - t0}ms`);
  }

  await client.detach();

  // Step 2: Select all and paste
  await page.keyboard.press("Control+A");
  await page.waitForTimeout(300);

  const t1 = Date.now();
  await page.keyboard.press("Control+V");
  log(`Ctrl+V paste completed in ${Date.now() - t1}ms`);
  ...
  await screenshot(page, "clipboard-filled.png");
  ...
  if (vis) {
    ...
    await screenshot(page, "clipboard-loaded.png");
    log("SUCCESS - indicator loaded");
  } else {
    log("Add to chart NOT visible");
    await screenshot(page, "clipboard-no-btn.png");
  }
} catch (e) {
  log(`FATAL: ${e.message.split("\n")[0]}`);
  await screenshot(page, "clipboard-error.png").catch(() => {});
} finally {
