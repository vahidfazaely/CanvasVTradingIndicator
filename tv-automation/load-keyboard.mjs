...
try {
  ...
  // Use keyboard.insertText — no CDP session, just keyboard events
  // This takes ~90s for 47K chars but completes reliably
  log("inserting text via keyboard.insertText (~90s)...");
  const t0 = Date.now();
  ...
  await screenshot(page, "keyboard-filled.png");
  ...
  if (vis) {
    ...
    await screenshot(page, "keyboard-loaded.png");
    log("SUCCESS - indicator loaded on chart");
  } else {
    log("Add to chart NOT visible");
    await screenshot(page, "keyboard-no-btn.png");
  }
} catch (e) {
  log(`FATAL: ${e.message.split("\n")[0]}`);
  await screenshot(page, "keyboard-error.png").catch(() => {});
} finally {
