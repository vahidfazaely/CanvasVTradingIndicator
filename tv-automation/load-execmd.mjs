...
try {
  ...
  // Use execCommand insertText — single synchronous call, handles any size
  const t0 = Date.now();
  await page.evaluate((text) => {
    document.execCommand("insertText", false, text);
  }, pineCode);
  log(`execCommand insertText completed in ${Date.now() - t0}ms`);

  await page.waitForTimeout(5000); // let Monaco process
  ...
  await screenshot(page, "execmd-filled.png");
  ...
  if (vis) {
    ...
    await screenshot(page, "execmd-loaded.png");
    log("SUCCESS - indicator loaded on chart");
  } else {
    log("Add to chart NOT visible");
    await screenshot(page, "execmd-no-btn.png");
  }
} catch (e) {
  log(`FATAL: ${e.message.split("\n")[0]}`);
  await screenshot(page, "execmd-error.png").catch(() => {});
} finally {
